...
// Ported from MboaTrustFrontend/src/api/inventoryItems.ts — identical field
// shapes/query keys; only the image-upload body differs, since RN has no
// `File` — expo-image-picker hands back a `{ uri, name?, mimeType? }` asset
// instead, appended to FormData RN's way (an object literal, not a Blob).
...
export interface InventoryItemInput {
  ...
  /** Newly picked images to upload alongside this save. */
  newImages?: PickedImage[];
}

/** Plain JSON when there are no new images (the common case for a field-only
 * edit), multipart only when newImages is non-empty — same rule as web. */
function toRequestBody(input: InventoryItemInput): FormData | Record<string, unknown> {
  ...
  newImages.forEach((img, i) => {
    ...
    // React Native's FormData accepts this shape in place of a Blob/File —
    // the RN networking layer streams directly from `uri` on send.
...
/** Single item, owner-only — used by the edit form. */
